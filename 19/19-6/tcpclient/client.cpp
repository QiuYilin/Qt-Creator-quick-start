    ...
    ui(new Ui::Client)
{
    ...
    connect(tcpClient, &QTcpSocket::bytesWritten, this, &Client::updateClientProgress);
    connect(tcpClient, &QTcpSocket::errorOccurred, this, &Client::displayError);
...
void Client::updateClientProgress(qint64 numBytes)
{
    ...
    // 更新进度条
    float temp = (bytesWritten / 1.0) / (totalBytes / 1.0);
    int value = temp * 100;
    ui->clientProgressBar->setMaximum(100);
    ui->clientProgressBar->setValue(value);
