    ...
    ui(new Ui::Widget)
{
    ...
    re.setPattern(R"((\w+))");
    QString subject("yafei linux qter");
    for (const QRegularExpressionMatch &match : re.globalMatch(subject)) {
        qDebug() << match.captured(1); // 结果为：yafei linux qter
    }
...
//    validator = new QRegularExpressionValidator(rx, this);
    ...
    re.setPattern("abc\\w+X|def");
    match = re.match("abcdef", 0, QRegularExpression::PartialPreferCompleteMatch);
    qDebug() << match.hasMatch();        // 结果为：true
    qDebug() << match.hasPartialMatch(); // 结果为：false
    qDebug() << match.captured(0);       // 结果为：def


    re.setPattern("abc\\w+X|defY");
    match = re.match("abcdef", 0, QRegularExpression::PartialPreferCompleteMatch);
    qDebug() << match.hasMatch();        // 结果为false
    qDebug() << match.hasPartialMatch(); // 结果为：true
    qDebug() << match.captured(0);       // 结果为：abcdef
...
{
    QRegularExpression rx("^[a-b]\\d{1,3}M$");
    QRegularExpressionMatch match = rx.match(arg1, 0,
